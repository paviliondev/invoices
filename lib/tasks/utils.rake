...
namespace :random do
  ...
  task :all do
    Rake::Task['random:invoices'].invoke
    Rake::Task['random:recurring_invoices'].invoke
  end
end

namespace :user do
  desc 'Creates user account with given credentials: rake user:create[name,email,pass]'
...
namespace :demo do
  ...
  task :setup, [:noinput] => :environment do |t, args|
    ...
    def setup_demo
      ...
      Rake::Task['random:all'].invoke
      Rake::Task['user:create'].invoke('demo', 'demo@example.com', 'secret')
    end
